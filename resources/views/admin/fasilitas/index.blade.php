        ...
        <div class="col-xs-12">
    		<section class="content-header">
         	...
         	<div class="box">
            	...
            	<div class="container">
                  {{ link_to('fasilitas/create','&nbsp; Tambah Data', ['id' => 'tambahFasilitas', 'class' => 'btn btn-success btn-lg btn-flat fa fa-plus', 'title' => 'Tambah']) }}
	            </div>
	            <br />
	                      <div class="col-sm-12">
...
@section('js')
	<script>
      	$(function() {
            var no = 0;
           	var table = $('#fasilitas').DataTable({
                ...
                processing: "<img src='{{ asset("loading/spinner.gif") }}' heigt='50' width='50'> Loading...",
           		...
           		processing: true,
               	serverSide: true,
               	ajax: "{{ url('dataFasilitas') }}",
                columns: [
                  { data: 'namaFasilitas', name: 'namaFasilitas'},
                  { data: 'detailFasilitas', name: 'detailFasilitas'},
                  { data: 'gambar', name: 'gambar', render: function(data,type,row){
                    return '<img src="gambarupload/'+data+'" height="100" width="300">';
                  }},
        	...
        	$.ajax({
            type: 'post',
            url: '{{ url('hapusFasilitas') }}',
            data: {
                '_token': "{{ csrf_token() }}",
                'id': $('.did').text()
            },
            success: function(data) { 
				      location.href = '{{ url('/fasilitas') }}';
            }
        	});
    });
      	});
